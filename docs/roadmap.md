...
3. Confirm Xbox 360 MSVC `16.0.10224.0` flags with small code-generation probes. **`/O2 /Oi` verified for thirty-one functions.**
...
Exit condition met: twenty-two source measurement units containing thirty-one functions are compiled with the target toolchain and have reproducible objdiff reports against their original COFFs. These units are not presented as proven original source-file boundaries.
