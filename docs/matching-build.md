...
A second probe passes all one hundred eighteen source-built objects to the linker. Its map
contains all two hundred five exact public functions. Because the remaining game code and
data are not reconstructed, this probe requires `/FORCE:UNRESOLVED` and reports
184 unique unresolved external symbols across 331 references. The linker emits a 20,480-byte
inspection PE with 15,560 bytes of `.text`; that section also includes eight
