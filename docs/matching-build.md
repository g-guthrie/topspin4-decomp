...
A second probe passes all one hundred twenty-one source-built objects to the linker. Its map
contains all two hundred thirteen exact public functions. Because the remaining game code and
data are not reconstructed, this probe requires `/FORCE:UNRESOLVED` and reports
188 unique unresolved external symbols across 337 references. The linker emits a 20,480-byte
inspection PE with 15,824 bytes of `.text`; that section also includes eight
