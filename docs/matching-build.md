...
The first eighty-five matching measurement units cover online feature stubs, session predicates, TSU statistics, profile summaries, friends leaderboards, Pro Challenge, title-server, player-visibility, minigame, King of the Court, TSU object callbacks, NetDB initialization, containers, and runtime helpers from the locked `Swing_DLL.xex`. Xbox 360 MSVC `16.00.10224.00` reproduces all 11,072 code bytes across ninety-five functions exactly: most units use `/O2 /Oi`, the per-round TSU aggregation and lobby-timer units additionally require `/GS-`, the three friends-leaderboard units require `/GS- /GR- /EHs-c-`, and selected runtime helpers require `/O1 /Oi`.
...
The match is 11,072 of 10,153,700 Swing code bytes (`0.109044000%`) and ninety-five of 47,250 objdiff functions (`0.201058210%`). The decomp-framework report has eighty-five complete-source units among 193 total units. These are matching-build counters, not the Ghidra discovery denominator in `config/progress.json`.
...
A second probe passes all eighty-five source-built objects to the linker. Its map
contains all ninety-five exact public functions. Because the remaining game code and
data are not reconstructed, this probe requires `/FORCE:UNRESOLVED` and reports
252 unresolved external symbols. The linker emits a 15,872-byte
inspection PE with 11,696 bytes of `.text`; that section also includes eight
