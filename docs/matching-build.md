...
A second probe passes all one hundred twenty-four source-built objects to the linker. Its map
contains all two hundred twenty-six exact public functions. Because the remaining game code and
data are not reconstructed, this probe requires `/FORCE:UNRESOLVED` and reports
201 unique unresolved external symbols across 350 references. The linker emits a 21,504-byte
inspection PE with 16,416 bytes of `.text`; that section also includes eight
