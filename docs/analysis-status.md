...
This is a discovery denominator, not a claim that every boundary is correct. It may change when bad disassembly is removed, missed functions are recovered, or thunks are normalized. The current reviewed counts are 34 named functions, one semantic source draft, one validated reconstruction, and one compiler-matched function.
...
Jeff `0.3.5` identified Xbox 360 MSVC C1/C2 `16.0.10224.0` and linker `10.0.10224.0` in all three modules. Official Jeff still fails on both DLLs; the reviewed repository patch now makes the complete Swing split succeed. The first source boundary and `/O1 /Oi` flags are verified for `src/xbox360/title_patch.c`, whose 56 code bytes match exactly. Flags and source-file boundaries remain unknown for the rest of the game, as does whole-program LTCG status. See [matching-decomp.md](matching-decomp.md), [matching-build.md](matching-build.md), and `config/toolchain.json`.
