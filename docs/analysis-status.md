...
This is a discovery denominator, not a claim that every boundary is correct. It may change when bad disassembly is removed, missed functions are recovered, or thunks are normalized. The current reviewed counts are 54 named functions, thirteen semantic source drafts, thirteen validated reconstructions, and thirteen compiler-matched functions.
...
Jeff `0.3.5` identified Xbox 360 MSVC C1/C2 `16.0.10224.0` and linker `10.0.10224.0` in all three modules. The reviewed repository patch now makes complete Swing and Loader splits succeed. Eleven measurement-unit boundaries and `/O2 /Oi` flags are verified, producing 1,344 exact code bytes across thirteen functions. Original source-file boundaries remain provisional for the game, as does whole-program LTCG status. See [matching-decomp.md](matching-decomp.md), [matching-build.md](matching-build.md), and `config/toolchain.json`.
