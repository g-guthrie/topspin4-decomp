...
This is a discovery denominator, not a claim that every boundary is correct. It may change when bad disassembly is removed, missed functions are recovered, or thunks are normalized. The current reviewed counts are 116 named functions, eighty semantic source drafts, eighty validated reconstructions, and eighty compiler-matched functions.
...
Jeff `0.3.5` identified Xbox 360 MSVC C1/C2 `16.0.10224.0` and linker `10.0.10224.0` in all three modules. The reviewed repository patch now makes complete Swing and Loader splits succeed. Seventy measurement-unit boundaries are verified, producing 9,676 exact code bytes across eighty functions; most use `/O2 /Oi`, the per-round TSU aggregation and lobby-timer callback additionally require `/GS-`, three friends-leaderboard units require `/GS- /GR- /EHs-c-`, and the thread-resume wrapper requires `/O1 /Oi`. Original source-file boundaries remain provisional for the game, as does whole-program LTCG status. See [matching-decomp.md](matching-decomp.md), [matching-build.md](matching-build.md), and `config/toolchain.json`.
