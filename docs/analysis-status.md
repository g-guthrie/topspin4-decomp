...
This is a discovery denominator, not a claim that every boundary is correct. It may change when bad disassembly is removed, missed functions are recovered, or thunks are normalized. The current reviewed counts are 72 named functions, twenty-nine semantic source drafts, twenty-nine validated reconstructions, and twenty-nine compiler-matched functions.
...
Jeff `0.3.5` identified Xbox 360 MSVC C1/C2 `16.0.10224.0` and linker `10.0.10224.0` in all three modules. The reviewed repository patch now makes complete Swing and Loader splits succeed. Twenty measurement-unit boundaries and `/O2 /Oi` flags are verified, producing 1,920 exact code bytes across twenty-nine functions. Original source-file boundaries remain provisional for the game, as does whole-program LTCG status. See [matching-decomp.md](matching-decomp.md), [matching-build.md](matching-build.md), and `config/toolchain.json`.
