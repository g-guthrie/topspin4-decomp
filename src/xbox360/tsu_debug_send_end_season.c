...
void ts4_online_send_end_season_to_all(
    void *online_context,
    ts4_script_u32 season
);

/*
 * Swing_DLL.xex 0x88787E10, registered as
 * TSU_Debug_SendEndSeasonToAll.
 */
ts4_script_u32 tsu_debug_send_end_season_to_all(
...
)
{
    ...
    if (g_ts4_online_context != 0) {
        ts4_online_send_end_season_to_all(
