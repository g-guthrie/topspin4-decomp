...
extern void ts4_pro_challenge_load_profile(void *service, int profile_index);

/* Swing_DLL.xex 0x8879B948, registered as ProChallenge_LoadProfile. */
u32 ts4_script_pro_challenge_load_profile(ScriptContext *context)
{
    ...
    if (service != 0) ts4_pro_challenge_load_profile(service, (int)profile_index);
    return 0;
}
