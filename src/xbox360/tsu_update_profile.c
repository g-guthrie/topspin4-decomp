...
ts4_script_u32 ts4_online_update_profile(
    void *online_context,
    int profile_index
);

/* Swing_DLL.xex 0x8879B9F8, registered as TSU_UpdateProfile. */
ts4_script_u32 tsu_update_profile(
...
)
{
    ...
    if (g_ts4_online_context != 0) {
        return ts4_online_update_profile(
