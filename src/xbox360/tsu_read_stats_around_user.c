...
extern void ts4_online_read_stats_around_user(
    void *online_context, u32 leaderboard_id, PlayerEntry *player,
    u32 count, int mode, u32 leaderboard_type
);

/* Swing_DLL.xex 0x8878B820, registered as TSU_ReadStatsAroundUser. */
u32 tsu_read_stats_around_user(ScriptContext *context)
{
    ...
    count = (u32)ts4_script_number(argument);
    argument = arguments->count > 3 ? arguments->first_value + 3 : default_value;
    mode = (int)ts4_script_number(argument);
    argument = arguments->count > 4 ? arguments->first_value + 4 : default_value;
    ...
    if (g_ts4_online_context != 0) {
        ...
        while (current != end) {
            ...
            if (player_index == player->player_index) {
                ts4_online_read_stats_around_user(
                    g_ts4_online_context, leaderboard_id, player, count, mode,
                    leaderboard_type
                );
                break;
            }
